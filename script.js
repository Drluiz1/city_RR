...
async function calcularRota(){
  ...
  const consumoCarro = parseFloat(document.getElementById('consumoCarro').value.replace(',', '.'));
  const pedagio = parseFloat(document.getElementById('pedagio').value.replace(',', '.') || 0);
  if(!origem || !destino || !precoCombustivel || !consumoCarro) return showToast('Preencha todos os campos da rota', 'error');
  ...
  try{
    ...
    const distanciaKm = coordOrigem && coordDestino ? map.distance(coordOrigem, coordDestino)/1000 : 0;
    const custoCombustivel = (distanciaKm / consumoCarro) * precoCombustivel;
    document.getElementById('resultadoRota').innerHTML = `
      <p><b>Distância aproximada:</b> ${distanciaKm.toFixed(2)} km</p>
      <p><b>Custo combustível:</b> R$ ${custoCombustivel.toFixed(2)}</p>
      <p><b>Pedágio:</b> R$ ${pedagio.toFixed(2)}</p>
      <p><b>Custo total estimado:</b> R$ ${(custoCombustivel+pedagio).toFixed(2)}</p>
    `;
  }catch(err){ showToast('Erro ao calcular rota', 'error'); }
}
